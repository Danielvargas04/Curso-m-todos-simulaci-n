...
//------------ PROGRAMMING ON THE HOST (CPU) ----------------
class LatticeBoltzmann{
private:
  float h_C[3];   // h_C[0]=C,  h_C[1]=C2,  h_C[2]=AUX0,
  float h_tau[3]; // h_tau[0]=tau,  h_tau[1]=Utau,  h_tau[2]=UmUtau,
  float h_w[Q];      //Weights  w[i]
  int h_Vx[Q],h_Vy[Q];  //Velocity vectors Vx[i],Vy[i]
  float *h_f,*h_fnew;  float *d_f,*d_fnew;// Distribution functions f[ix][iy][i]
public:
...
int main(void){
  ...
  //Show
  Ondas.Print("Ondas2D_2024-I_Con CUDA.dat");
