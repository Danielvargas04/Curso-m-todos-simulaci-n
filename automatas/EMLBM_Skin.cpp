...
LatticeBoltzmann::LatticeBoltzmann(void){
  ...
  v0.load(V0[0],V0[1],V0[2]); //load= load (in Spanish)
  for(p=0;p<3;p++)
    for(i=0;i<4;i++){
...
vector3D LatticeBoltzmann::D(int ix,int iy,int iz,bool UseNew){
  int p,i,j; vector3D sum; sum.load(0,0,0);
  for(p=0;p<3;p++)
    for(i=0;i<4;i++)
      for(j=0;j<2;j++)
	if(UseNew) 
...
void LatticeBoltzmann::Start(void){
  ...
  for(ix=0;ix<Lx;ix++) //para cada celda
	...
	//Impose the fields
	rhoc0=0.0; Jprime0.load(0,0,0); 
  B0.load(0, B00*exp(-0.25*(iz-iz0)*(iz-iz0)/(alpha0*alpha0)),0);
  Eprime0.load(E00*exp(-0.25*(iz-iz0)*(iz-iz0)/(alpha0*alpha0)),0,0);
...
void LatticeBoltzmann::ImposeFields(int t){
  ...
  for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++){
      ...
      D0.load(E00*sin(omega*t)*epsilonr0,0,0);
      B0.load(0,B00*sin(omega*t),0);
