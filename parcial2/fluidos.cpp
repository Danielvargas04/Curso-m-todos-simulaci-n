...
const double sigma=Ly/9, mu=Lx/8;
...
{
    double dp=0.001, nu=10.0, rho0;
    rho0=rho(ix,iy,UseNew);
    return rho0/2*dp/nu*iy*(Ly-1-iy);
}
...
{
    std::cout<<"set title 'Densidad en funcion del tiempo'"<<std::endl;
    std::cout<<"splot '-' using 1:2:3 with pm3d notitle "<<std::endl;
...
void LatticeBoltzmann::map_density(void)
{
    std::cout<<"set pm3d map "<<std::endl;
    std::cout<<"set terminal jpeg enhanced"<<std::endl;
    std::cout<<"set output 'Mapa de densidad.jpg'"<<std::endl;
    ...
    std::cout<<"set cblabel 'Densidad'"<<std::endl;
    std::cout<<"set title 'Densidad en el tiempo'"<<std::endl;  
}

void LatticeBoltzmann::Plot(void)
{
    std::cout << "set terminal jpeg enhanced" << std::endl;
    std::cout << "set output 'Detector.jpg'" << std::endl;
    std::cout << "set xlabel 'Tiempo'" << std::endl;
    std::cout << "set ylabel 'Densidad total'" << std::endl;
    std::cout << "set title 'Densidad en el punto x = 120'" << std::endl;
    std::cout << "plot '-' using 1:2 with points title 'Datos Simulacion'"<< std::endl;  
}
void LatticeBoltzmann::Detector(int t)
{
    double medida;
    for (int iy = 0; iy < Ly; iy++)
    {
        medida+=rho(120,iy,true);
    }
    std::cout<<t<<" "<<medida<<std::endl;
}

int main(void)
{
    LatticeBoltzmann air;
    int taux=0, t, tmax=5000;
    double varianza;
